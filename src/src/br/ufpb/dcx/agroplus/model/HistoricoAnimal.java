...
public class HistoricoAnimal {
    ...
    private List<ProducaoDiaria> listaProducaoAssociada;

    public HistoricoAnimal(Animal animal, List<RegistroPeso> listaPesos,
                           List<AplicacaoProduto> listaAplicacoes,
                           List<ProducaoDiaria> listaProducaoAssociada) {
        this.animal = animal;
        ...
        if (listaAplicacoes.isEmpty()) {
            ...
            for (AplicacaoProduto ap : listaAplicacoes) {
                sb.append(String.format("- Produto ID: %d | Dose: %s | Data: %s | Aplicado por: %s | Obs: %s\n",
                        ap.getProdutoId(), ap.getDose(), ap.getDataAplicacao(),
                        ap.getFuncionarioResponsavel(), ap.getObservacoes()));
            }
        }

        sb.append("\n--- Produção Associada ao Lote (").append(animal.getLote()).append(") ---\n");
