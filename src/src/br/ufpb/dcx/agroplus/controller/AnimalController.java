...
public class AnimalController {
    ...
    public void cadastrarAnimal(String identificacaoUnica, String especie, int idadeMeses, float peso, String saude, String lote) {
        ...
        Animal novo = new Animal(repository.getNextIdAnimal(), identificacaoUnica, especie, idadeMeses, peso, saude,
            LocalDate.now().minusMonths(idadeMeses), lote);
        repository.addAnimal(novo);
        repository.addRegistroPeso(new RegistroPeso(repository.getNextIdPeso(), novo.getId(), peso, LocalDate.now()));
    ...
    public void atualizarPeso(int animalId, float novoPeso) {
        ...
        repository.addRegistroPeso(new RegistroPeso(repository.getNextIdPeso(), a.getId(), novoPeso, LocalDate.now()));
    ...
    public HistoricoAnimal obterHistorico(int animalId) {
        ...
        if (a == null) {
            for (Animal animal : repository.getAnimais()) {
                if (animal.getId() == animalId) { a = animal; break; }
            }
        }
        if (a == null) throw new IllegalArgumentException("Animal não encontrado no banco de dados.");

        List<RegistroPeso> listaPesos = new ArrayList<>();
        for (RegistroPeso rp : repository.getPesos()) if (rp.getAnimalId() == a.getId()) listaPesos.add(rp);
        List<AplicacaoProduto> listaAplicacoes = new ArrayList<>();
        for (AplicacaoProduto ap : repository.getAplicacoes()) if (ap.getAnimalId() == a.getId()) listaAplicacoes.add(ap);
        List<ProducaoDiaria> listaProd = new ArrayList<>();
        for (ProducaoDiaria pd : repository.getProducoes()) if (pd.getLote().equalsIgnoreCase(a.getLote())) listaProd.add(pd);
        return new HistoricoAnimal(a, listaPesos, listaAplicacoes, listaProd);
    }

    public Animal encontrarPorId(int id) {
        for (Animal a : repository.getAnimais()) if (a.getId() == id && a.isAtivo()) return a;
        return null;
    }
}
